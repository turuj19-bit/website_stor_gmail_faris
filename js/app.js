...
(function() {
  ...
  if (typeof originalApiRequest === 'function') {
    window.apiRequest = async function(action, data) {
      // Jika action adalah createStor, pastikan ada field jenis
      if (action === 'createStor' && data && typeof data === 'object') {
        // Jika data.jenis tidak ada, deteksi dari form yang aktif
        if (!data.jenis) {
          // Coba deteksi dari form yang aktif
...
(function() {
  ...
  window.loadSettings = function() {
    ...
    // Ambil dari appState jika tersedia
...
let showChoiceMode = true; // true = tampilkan dua card, false = tampilkan form
...
function updateUIApp() {
  try {
    ...
    // ===== UPDATE STATUS STORAN (gabungan) untuk Beranda =====
    ...
    // ===== Harga dan password hanya untuk Fresh (tetap di Beranda) =====
    ...
    if (storHarga) storHarga.textContent = 'Rp ' + formatRupiah(settings.depositPrice || 4000);
    ...
    // ===== INISIALISASI CARD STOR =====
    initStorCards();

    // ===== Jika di halaman stor, pastikan tampilan choice/form sesuai =====
    if (currentPage === 'stor') {
      if (showChoiceMode) {
        showStorChoice();
      } else {
        // tetap tampilkan form yang sedang aktif, tidak direset
      }
    }
...
window.addEventListener('popstate', function() {
  ...
  // Jika di halaman stor dan sedang di choice mode, tidak perlu kembali
  // Jika di halaman stor dan sedang di form, kembali ke choice
...
function showPage(page) {
  ...
  if (needRefresh.includes(page) && getToken()) {
    refreshAppState().then(() => {
      ...
      if (page === 'stor') {
        // Tampilkan choice mode saat masuk ke stor
        showChoiceMode = true;
        showStorChoice();
        updateStatsStor();
      }
...
function showStorChoice() {
  ...
  const backBtn = document.getElementById('storBackBtn');
  if (choiceContainer) choiceContainer.style.display = 'flex';
  if (formFresh) formFresh.style.display = 'none';
  if (formBekas) formBekas.style.display = 'none';
  if (backBtn) backBtn.style.display = 'none';
  // Reset form values jika perlu
  // Tidak direset agar data tidak hilang saat kembali
}
...
function showFreshForm() {
  ...
  if (formFresh) formFresh.style.display = 'block';
  if (formBekas) formBekas.style.display = 'none';
...
function showBekasForm() {
  ...
  if (choiceContainer) choiceContainer.style.display = 'none';
  if (formFresh) formFresh.style.display = 'none';
  if (formBekas) formBekas.style.display = 'block';
  if (backBtn) backBtn.style.display = 'flex';
  setTanggalBekas();
  updateCounterBekas();
  pushOverlayHistoryGuard();
...
function updateStorCards() {
  ...
  if (freshCard) {
    ...
    if (freshNameEl) freshNameEl.textContent = freshName;
  ...
  // Tampilkan status closed pada form (jika form sedang ditampilkan, update pesan)
  const freshClosedMsg = document.getElementById('storClosedMsg');
  if (freshClosedMsg) {
    freshClosedMsg.style.display = (freshStatus !== 'OPEN') ? 'block' : 'none';
  }
  const bekasClosedMsg = document.getElementById('storBekasClosedMsg');
  if (bekasClosedMsg) {
    bekasClosedMsg.style.display = (bekasStatus !== 'OPEN') ? 'block' : 'none';
  }
}
...
// ============================================================
// SUBMIT STOR FRESH (tetap seperti lama)
// ============================================================
async function submitStor(type) {
  // Jika type tidak diberikan, deteksi otomatis dari form yang aktif
  if (!type) {
    const freshForm = document.getElementById('storFormFresh');
    const bekasForm = document.getElementById('storFormBekas');
    if (bekasForm && bekasForm.style.display !== 'none') {
