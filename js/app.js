...
function showPage(page) {
  try {
    ...
    // FIX BUG: layar pilihan Stor (Fresh/Bekas) sekarang di-reset SEKALI
    // saja di sini, langsung/synchronous, persis saat menu Stor pertama
    // kali dibuka (bukan saat menu Stor yang sedang aktif ditekan ulang).
    // Sebelumnya reset ini juga dipanggil ULANG di dalam
    // refreshAppState().then(...) di bawah, yang baru selesai belakangan
    // (setelah request ke server). Kalau dalam jeda itu user keburu tap
    // kartu Fresh/Bekas (form sudah kebuka), reset yang telat itu akan
    // menutup form itu lagi & menyeret balik ke layar pilihan secara
    // tiba-tiba -- inilah bug "kembali sendiri" yang dilaporkan. Sekarang
    // reset hanya terjadi sekali di awal saat halaman Stor pertama kali
    // dibuka, sehingga pilihan Fresh/Bekas oleh user tidak akan pernah
    // ditimpa lagi oleh proses refresh data yang berjalan di belakang layar.
...
// FIX: sebelumnya di sini closeModal('modalConfirmBackStor') dipanggil
// bersamaan dengan goBackToStorChoice() (yang juga mundur 1 langkah history
// lewat popOverlayHistoryGuard). Keduanya sama-sama mundur di history browser
// hampir bersamaan, jadi baliknya "dobel" dan salah satu langkah balik itu
// lolos tanpa ke-handle dengan benar, akibatnya malah nyasar ke Beranda.
// Sekarang modal ini ditutup langsung (tanpa lewat closeModal, cukup lepas
// class 'open'-nya), lalu goBackToStorChoice() yang urus baliknya, cuma 1x.
