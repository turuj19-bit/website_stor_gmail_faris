...
// ============================================================
// ============================================================
// KONFIGURASI TETAP: Gmail Bekas Biasa & Gmail Bekas Old
// ============================================================
// CATATAN PENTING: harga & status OPEN/CLOSE 2 jenis storan ini BELUM
// bisa diatur dari dashboard admin (beda dengan Fresh/Bekas), karena
// dashboard admin nyimpen pengaturan lewat fungsi khusus di database
// Supabase (panel_update_settings) yang cuma "kenal" kolom Fresh & Bekas.
// Nambah kolom baru di situ butuh update di sisi database (di luar file
// ini), jadi untuk sementara diatur manual di sini dulu.
// -> Mau ganti harga / buka-tutup storan Biasa & Old? Ubah angka/nilai
//    di bawah ini saja.
const STOR_BIASA_CONFIG = { status: 'OPEN', name: 'Gmail Bekas Biasa', price: 2500, info: 'Bekas biasa, tanpa 2FA' };
const STOR_OLD_CONFIG   = { status: 'OPEN', name: 'Gmail Bekas Old',   price: 2000, info: 'Akun lama, sudah lama dibuat' };
...
function updateStorCards() {
  try {
    ...
    // FIX: kartu Bekas Biasa & Bekas Old (pengaturan tetap, lihat
    // STOR_BIASA_CONFIG / STOR_OLD_CONFIG di atas)
    [
      { cfg: STOR_BIASA_CONFIG, prefix: 'storCardBiasa', closedMsg: 'storBiasaClosedMsg' },
      { cfg: STOR_OLD_CONFIG, prefix: 'storCardOld', closedMsg: 'storOldClosedMsg' }
...
function initStorCards() {
  ...
  try {
    ...
    // FIX: kartu Bekas Biasa & Bekas Old (lihat STOR_BIASA_CONFIG/STOR_OLD_CONFIG)
    if (biasaCard) {
      biasaCard.addEventListener('click', function(e) {
        if (STOR_BIASA_CONFIG.status !== 'OPEN') {
    ...
    if (oldCard) {
      oldCard.addEventListener('click', function(e) {
        if (STOR_OLD_CONFIG.status !== 'OPEN') {
