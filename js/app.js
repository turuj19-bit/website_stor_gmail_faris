...
function closeWelcomeToast() {
  ...
  // FIX: setelah popup "Selamat Datang" ditutup, baru pindah dari halaman
  // login (authPage) ke halaman utama (appPage). Sebelumnya enterApp()
  // tidak pernah dipanggil sehingga user tetap terjebak di halaman login.
  const authPage = safeGet('authPage');
  if (authPage && !authPage.classList.contains('hide')) {
    enterApp();
  }
}
