  ...
  if (typeof originalApiRequest === 'function') {
    window.apiRequest = async function(action, data) {
      ...
      if (action === 'createStor' && data && typeof data === 'object') {
        // Jika data.jenis tidak ada, deteksi dari data atau gunakan default 'fresh'
        if (!data.jenis) {
...
(function() {
  ...
  window.loadSettings = function() {
    ...
    if (window.appState && window.appState.settings) {
      ...
      settings.depositBekasName   = window.appState.settings.depositBekasName || settings.depositBekasName || 'Gmail Bekas';
...
// --- FUNGSI UTAMA SUBMIT STOR (DIPERBAIKI) ---
async function submitStor(type) {
  ...
  try {
    const result = await apiRequest('createStor', {
      ...
      jenis: type  // <-- KIRIM JENIS KE SERVER (juga akan ditambahkan oleh override apiRequest)
    });
