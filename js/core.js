// core.js — State/helper global: session/token, apiRequest, musik, modal generik, utilitas (formatRupiah, formatTime, showToast)
    ...
    // ============================================================
    // HALAMAN TERAKHIR (dipakai supaya refresh tidak selalu balik ke
    // Beranda -- lihat pemakaian di showPage() [app.js] & init.js)
    // ============================================================
    const LAST_PAGE_KEY = 'storfaris_last_page';
    function getSavedPage() {
      try { return localStorage.getItem(LAST_PAGE_KEY) || 'beranda'; } catch (_) { return 'beranda'; }
    }
    ...
    async function apiRequest(action, data = {}, authenticated = true, timeoutMs = 20000) {
      ...
      // ===== PERBAIKAN: PASTIKAN p_jenis TERKIRIM UNTUK createStor =====
      if (action === 'createStor') {
        // Jika params belum punya p_jenis, ambil dari data atau deteksi otomatis
        if (!params.p_jenis) {
          if (data.jenis) {
            params.p_jenis = data.jenis;
          } else {
            // Fallback: cek form mana yang aktif
            ...
            if (bekasForm && bekasForm.classList.contains('show')) {
              params.p_jenis = 'bekas';
            } else if (freshForm && freshForm.classList.contains('show')) {
              params.p_jenis = 'fresh';
            } else {
              // Terakhir, cek display style (kompatibilitas)
        ...
        console.log('[apiRequest] createStor params:', params);
      }
    ...
    async function refreshAppState() {
      ...
      try {
        ...
        // PERBAIKAN: sebelumnya kegagalan APA PUN di sini (termasuk cuma
        // koneksi lambat/putus sesaat atau timeout saat refresh halaman)
        // dianggap sama dengan sesi tidak valid, lalu token langsung
        // dihapus paksa di init.js -> user ke-logout sendiri walau
        // sesinya sebenarnya masih sah. Sekarang token HANYA dihapus
        // kalau pesan errornya memang menandakan sesi/token tidak valid
        // atau kedaluwarsa. Untuk error lain (jaringan/timeout/server
        // lambat), token tetap disimpan supaya bisa dicoba lagi otomatis
        // (lihat interval refresh di init.js) tanpa perlu login ulang.
        const msg = (e && e.message) || '';
        if (/sesi|session|token|kedaluwarsa|expired|invalid/i.test(msg)) {
          clearToken();
          appState.user = null;
        }
        return false;
